...
##### ##### ##### #####
# For our iMixer experiments
group = parser.add_argument_group('Added parameters for iMixer experiments')
group.add_argument('--h_ratio', type=float, default=1.0,
                    help='dimension of hidden neurons relative to internal neurons. (default: 1.0)')
group.add_argument('--n_power', type=int, default=8, metavar='N',
                    help='number of power iterations in the spectral normalization. (default: 8)')
group.add_argument('--n_iter', type=int, default=4, metavar='N',
                    help='number of iterations for the fixed-point iteration method. (default: 4)')
##### ##### ##### #####
...
def main():
    ...
    model = create_model(
        ...
        hidden_ratio=args.h_ratio,
        n_power_iterations=args.n_power,
        n_repeat=args.n_iter,
    ...
    if args.distributed:
        if has_apex and use_amp == 'apex':
            ...
            model = NativeDDP(model, device_ids=[args.local_rank], broadcast_buffers=not args.no_ddp_bb) #, find_unused_parameters=True)  # add `find_unused_parameters=True` for imixer-simplest, 5/Dec/2022
